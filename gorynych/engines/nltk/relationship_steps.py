...
with open(os.path.join(gorynych.__path__[0], 'dicts', 'relations.json'), 'r') as r:
...
def get_raw_relstrings(ne_chunks):
    semi_rel = nltk.sem.relextract.semi_rel2reldict(nltk.sem.relextract.tree2semi_rel(ne_chunks))
    result = []
    for rel in semi_rel:
        for rel_type in RELATIONS:
